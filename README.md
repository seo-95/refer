...
## Prepare Images:
Besides, add "mscoco" which can be from [mscoco](http://mscoco.org/dataset/#overview)
COCO's images are used for RefCOCO, RefCOCO+ and refCOCOg.
For RefCLEF, we extracted the related 19997 images to our cleaned RefCLEF dataset, which is a subset of the original [imageCLEF](http://imageclef.org/SIAPRdata). Download the [subset](http://bvisionweb1.cs.unc.edu/licheng/referit/data/images/saiapr_tc-12.zip)
...
refer = REFER(imgDir, instancesDir, dataset='refclef', splitBy='unc')
refer = REFER(imgDir, instancesDir, dataset='refclef', splitBy='berkeley') # 2 train and 1 test images missed
refer = REFER(imgDir, instancesDir, dataset='refcoco',  splitBy='unc')
refer = REFER(imgDir, instancesDir, dataset='refcoco',  splitBy='google')
refer = REFER(imgDir, instancesDir, dataset='refcoco+', splitBy='unc')
refer = REFER(imgDir, instancesDir, dataset='refcocog', splitBy='google')   # test split not released yet
refer = REFER(imgDir, instancesDir, dataset='refcocog', splitBy='umd')      # Recommended, including train/val/test
```
